...
namespace MixCaptcha;

use think\facade\Session;
use think\Response;
const ZH_TTF_DIR = __DIR__ . '/assets/zhttfs/';
const EN_TTF_DIR = __DIR__ . '/assets/ttfs/';
 ...
 * @property int    $useZh    使用中文验证码 0-不用中文 1-只用中文 2-中英文数字混用
...
class MixCaptcha
{
    ...
    public function __construct($config = [])
    {
        ...
        $this->zhTTFList = $this->getTTFListFromDir(ZH_TTF_DIR);
        $this->enTTFList = $this->getTTFListFromDir(EN_TTF_DIR);
    }
    ...
    private function getTTFListFromDir($dirPath)
    {
        $dir = dir($dirPath);
        $ttfList = [];
        while (false !== ($file = $dir->read())) {
            if ('.' != $file[0] && substr($file, -4) == '.ttf') {
                $ttfList[] = $dirPath . $file;
            }
        }
        $dir->close();
        return $ttfList;
    ...
    private function getTTF($zh = false)
    {
        if ($this->fontttf) {
            return $this->fontttf;
        }
        ...
        if ($zh) {
            $zhTTFList = $this->zhTTFList;
            return $zhTTFList[array_rand($zhTTFList)];
        }

        $enTTFList = $this->enTTFList;
        return $enTTFList[array_rand($enTTFList)];
    }
    ...
    public function generate()
    {
        ...
        for ($i = 0; $i < $this->length; $i++) {
            if ($this->useZh === 0) {
                $thisUseZh = false;
            } else if ($this->useZh === 1) {
                $thisUseZh = true;
            } else {
                $thisUseZh = mt_rand(0, 1) === 0;
            }

            $ttf = $this->getTTF($thisUseZh);

            if (!$thisUseZh) {
                $code[$i] = $this->codeSet[mt_rand(0, strlen($this->codeSet) - 1)];
                $codeNX += mt_rand($this->fontSize * 1.2, $this->fontSize * 1.6);
                imagettftext($this->im, $this->fontSize, mt_rand(-40, 40), $codeNX, $this->fontSize * 1.6, $this->color, $ttf, $code[$i]);
            } else {
                $code[$i] = iconv_substr($this->zhSet, floor(mt_rand(0, mb_strlen($this->zhSet, 'utf-8') - 1)), 1, 'utf-8');
                $codeNX += $this->fontSize * 1.4;
                imagettftext($this->im, $this->fontSize, mt_rand(-40, 40), $codeNX, $this->fontSize + mt_rand(10, 20), $this->color, $ttf, $code[$i]);
            }
        }
    ...
    public function entry($id = '')
    {
        ...
        $code = $result['code'];
        $image = $result['image'];
        $key = $this->hashCode($this->seKey);
        $code = $this->hashCode(strtoupper($code));
        $secode = [];
    ...
    {
        $key = $this->hashCode($this->seKey) . $id;
        ...
        if ($this->hashCode(strtoupper($code)) == $secode['verify_code']) {
